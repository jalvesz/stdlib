...
contains
    ...
    subroutine test_io_mm_coordinate(error)
        ...
        ! Validate given matrices inside https://math.nist.gov/MatrixMarket/data/SPARSKIT/
        block
            real(dp), allocatable :: data_f_load(:), data_s_load(:)
            ...
            call load_mm(path // "fidap005.mtx", index_f_load, data_f_load, iostat=stat)
            nnz = size(index_f_load, dim=2)
            call load_npy(path // "fidap005.mtx_indices.npy", index_s_load, iostat=stat)
            index_s_load = index_s_load + 1
            call load_npy(path // "fidap005.mtx_data.npy", data_s_load, iostat=stat)
            call check(error, all(index_f_load(1,:)==index_s_load(1:nnz)), .true.,&
                "MM coordinate test failed: fidap005.mtx rows not matched")
            call check(error, all(index_f_load(2,:)==index_s_load(nnz+1:2*nnz)), .true.,&
                "MM coordinate test failed: fidap005.mtx cols not matched")
            call check(error, all_close(data_f_load, data_s_load), .true.,&
                "MM coordinate test failed: fidap005.mtx data not matched")
        end block
        stat = 0
