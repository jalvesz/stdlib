...
contains
    ...
    subroutine generate_random_for_sp_dense(A)
    ...
    subroutine generate_random_for_dp_dense(A)
    ...
    subroutine generate_random_for_csp_dense(A)
    ...
    subroutine generate_random_for_cdp_dense(A)
    ...
    subroutine generate_random_for_int8_dense(A)
    ...
    subroutine generate_random_for_int16_dense(A)
    ...
    subroutine generate_random_for_int32_dense(A)
    ...
    subroutine generate_random_for_int64_dense(A)
    ...
    subroutine test_io_mm_array(error)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_sp_dense(matrix_save)
            ...
            ! Symmetric matrix
            call generate_random_for_sp_dense(A)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_dp_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_dp_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_csp_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_csp_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_cdp_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_cdp_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_int8_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_int8_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_int16_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_int16_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_int32_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_int32_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_for_int64_dense(matrix_save)
            call save_mm("test_mmio_dense.mtx", matrix_save, format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
            ...
            ! Symmetric matrix
            call generate_random_for_int64_dense(A)
            ! Construct symmetric matrix using (A + A.T)
            matrix_save = A + transpose(A)
            call save_mm("test_mmio_dense.mtx", matrix_save, symmetry = "symmetric", format = "G0")
            call load_mm("test_mmio_dense.mtx", matrix_load)
    ...
    subroutine generate_random_for_sp_coo(A, nnz_to_write)
        real(sp), intent(out) :: A(:)
    ...
    subroutine generate_random_positions_sp(pos, t_entries)
    ...
    subroutine generate_random_positions_dp(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine generate_random_positions_csp(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine generate_random_positions_cdp(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine generate_random_positions_int8(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine generate_random_positions_int16(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine generate_random_positions_int32(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine generate_random_positions_int64(pos, t_entries)
        integer, intent(inout) :: pos(:)
        integer, intent(in) :: t_entries
    ...
    subroutine test_io_mm_coordinate(error)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_sp(pos, t_entries)
            ...
            call generate_random_for_sp_coo(data_save, nnz)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_dp(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_dp_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
        ...
        block
            ...
            ncols = 5
            t_entries = nrows * ncols
            allocate(R(t_entries, 2))
            allocate(pos(t_entries))

            ! General matrix
            call generate_random_positions_csp(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_csp_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_cdp(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_cdp_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_int8(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_int8_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_int16(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_int16_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_int32(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_int32_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
        ...
        block
            ...
            ! General matrix
            call generate_random_positions_int64(pos, t_entries)
            call random_number(density)
            nnz = ceiling(density*t_entries)
            allocate(index_save(2, nnz))
            allocate(data_save(nnz))
            ...
            call generate_random_for_int64_coo(data_save, nnz)
            call save_mm("test_mmio_sparse.mtx", index_save, data_save, format = "G0")
            call load_mm("test_mmio_sparse.mtx", index_load, data_load)
